...
- **90-100 分** (紅區 - 致命): 觸發熔斷（如：破產紀錄、詐欺前科、負責人限制出境、欠稅大戶）
...
#### 🟠 MAA (互相保證消耗/承諾深度指標) - "雙方押了多少籌碼（資源鎖定）？"
**範式轉變**：MAA 不再是"行政內耗"（越高越差），而是"互相保證消耗"（承諾深度，越高越穩定）

衡量雙方為維持關係而保證投入的不可撤銷資源（0-100 分，越高代表綁定越深/越穩定）

- **0-20 分** (流動式交易): 隨用隨棄。無低消、無訂金、窗口隨機
- **21-50 分** (預約制維護): 基本資源預留。有預付款/訂金、定期會議、指定窗口
- **51-80 分** (硬性鎖定): 違約成本高。保證採購/產能、沉沒成本投入、高額解約金、利潤綁定
- **81-100 分** (共生/排他): 命運共同體。獨家條款、股權互持、核心命脈託管

**重要**：標準行政作業應給 20-30 分（綠區），不要因「有人工流程」就扣分。高 MAA 代表穩定的護城河。
...
例子：
- MAD=30, MAO=70, MAA=85, MAP=75
  → 總分 = (70×1.0) + (85×0.5) + (75×0.8) - 30
  → 總分 = 70 + 42.5 + 60 - 30 = 142.5
  → 標準化到 0-100: ((142.5 + 89) / 319) × 100 ≈ 73 分

特殊規則：
- 如果 MAD ≥ 90 (死亡區 - Blockers)，觸發熔斷機制，總分 = 0
